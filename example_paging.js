$scope.getAllData = function(layout, handle, lastRow, callbackFn){ 
    if(!layout.qHyperCube){ callbackFn.call(); } 
    if(lastRow==0){ layout.qHyperCube.qDataPages = []; } 
    var pages = [{ qTop: lastRow, qLeft: 0, qHeight: 100, qWidth: 10 }]; 
    $scope.session.rpc(
            {handle: handle, method: "GetHyperCubeData", 
